...
Queue<ScheduledState> stateQueue = Queue<ScheduledState>(10);
void scheduleState(uint8_t state, unsigned long delayMillis) {
    ScheduledState scheduled = { state, millis() + delayMillis };
    stateQueue.push(scheduled);
    if (DEBUG) {
        Serial.print("State scheduled: ");
        Serial.print(state);
        Serial.print(" at ");
